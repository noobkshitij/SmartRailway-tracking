...
void setup() {
  ...
  pinMode(13, OUTPUT); // Buzzer or extra LED
...
void loop() {
  ...
  // 🚨 Traffic Light Logic
  if (distance1 < 7) {  // Train arriving
    digitalWrite(RED_LIGHT, HIGH);    // Red ON
    digitalWrite(GREEN_LIGHT, LOW);   // Green OFF
  }
  else if (distance2 < 7) {  // Train departing
    digitalWrite(RED_LIGHT, LOW);     // Red OFF
    digitalWrite(GREEN_LIGHT, HIGH);  // Green ON
  }
  else {
  ...
  if (distance1 < 7 || distance2 < 7) {   // Object detected
    if (gateOpen) {
      ...
      myservo1.write(55);
      myservo.write(144);
      digitalWrite(13, HIGH); // buzzer or extra LED
      delay(750);
    }
  } else {                                // No object
    if (!gateOpen) {
